 ...
 * En el taller de Santa 🎅 se están preparando los trineos de motor eléctrico para poder hacer la ruta perfecta para dejar los regalos.
    ...
    La dificultad del reto está en pensar que sólo podemos configurar el salto del trineo una vez y que buscamos el salto mínimo 
    que nos serviría para sortear todos los obstaculos.
 */
...
export default function getMinJump(obstacles) {
  ...
  for (let jump = minJump; jump < 99; jump++) {
    collision = false;
    for (let i = 0; i <= 99; i = i + jump) {
      if (i > 0) {
        if (obstacles.find((obs) => obs == i)) {
          collision = true;
          break;
        }
      }
