...
export default function shouldBuyFidelity(times) {
  return fidelityTickets(times) < normalTickets(times);
}

function normalTickets(times) {
  return normalPrice * times;
}

function fidelityTickets(times) {
  const arrayTimes = Array.from({ length: times }, (_, i) => i + 1);
  const reducer = (total, ticket) => {
    return total + normalPrice * Math.pow(0.75, ticket);
  };
  return arrayTimes.reduce(reducer, fidelityCardPrice);
}
